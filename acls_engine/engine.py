...
from algorithms.megacode import MegacodeAlgorithm

# ── Performance Tracker (optional — skips gracefully if file missing) ─────────
try:
    from performance_tracker import ingest_session as _tracker_ingest
...
class ACLSEngine:
    ...
    def save_findings(
        ...
        output: Dict = {
            ...
            "phase_report": (
                self._processor.get_phase_report()
                if hasattr(self._processor, "get_phase_report") else None
            ),
        }
...
if __name__ == "__main__":
    scenario_file = sys.argv[1] if len(sys.argv) > 1 else "test_scenarios/megacode_case1.json"
    ...
    # ── Auto-run performance tracker ──────────────────────────────────────────
    if _TRACKER_AVAILABLE:
        logger.info("\n[TRACKER] Ingesting session into performance_db.json ...")
        try:
            _tracker_ingest(
                findings_path=output_file,
                scenario_path=scenario_file
            )
        except Exception as e:
            logger.warning(f"[TRACKER] Could not ingest session: {e}")
    else:
        logger.info("[TRACKER] performance_tracker.py not found — skipping.")
